...
export async function resolveSlackConversationInfo(params: {
  ...
  const workspaceId = normalizeOptionalString(params.workspaceId)?.toUpperCase();
  const cacheKey = `${account.accountId}:${operation}:${credentialRole}:${credentialFingerprint}:${workspaceId ?? "workspace-default"}:${channelId}`;
  ...
  if (token) {
    try {
      ...
      if (isNativeImChannel && operation === "write") {
        const client = createSlackWebClient(token, workspaceId ? { teamId: workspaceId } : {});
      ...
      const client = createSlackReadClient(token, workspaceId ? { teamId: workspaceId } : {});
