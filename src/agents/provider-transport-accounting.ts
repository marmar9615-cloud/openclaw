...
function retainEventIdentity(
  ...
  state.eventFingerprints.set(event.eventId, {
...
function sealPendingSettlement(
  ...
  if (evidence !== "failed" || settlementJustArrived || call.latestZeroSubmissionOutcome) {
...
function applyAttempt(
  ...
  if (!call || !canContinueAfterAttempt(call, state)) {
    return false;
  }
...
export function createProviderTransportAccountingCollector(): ProviderTransportAccountingCollector {
  ...
  const observer: ProviderTransportAccountingObserver = {
    onLogicalCallStarted(rawCall) {
      ...
      if (existing) {
        if (!sameRequestedRoute(existing, route)) {
