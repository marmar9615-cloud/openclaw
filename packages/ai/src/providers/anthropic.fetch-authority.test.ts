...
describe("Anthropic SDK endpoint authority and injected clients", () => {
  it.each(["hostless", "legacy"] as const)(
    ...
    async (hostMode) => {
      ...
      expect(events).not.toEqual(
        expect.arrayContaining([expect.objectContaining({ type: "provider_fallback" })]),
      );
    },
  );
  ...
  it("accepts structurally complete clean EOF from an injected compatible client as partial", async () => {
    const events: AiModelTransportEvent[] = [];
    const fetchMock = vi.fn<typeof fetch>(
      async () =>
        new Response(
