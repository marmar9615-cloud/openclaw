...
async function* iterateAnthropicEvents(
  ...
  const events = Stream.rawEvents(response)[Symbol.asyncIterator]();
  let completed = false;
  try {
    while (true) {
      ...
      if (!ANTHROPIC_MESSAGE_EVENTS.has(sse.event ?? "")) {
        try {
          const unlabelledEvent = parseJsonWithRepair(sse.data) as { type?: unknown };
          if (
            typeof unlabelledEvent.type === "string" &&
            ANTHROPIC_MESSAGE_EVENTS.has(unlabelledEvent.type)
          ) {
            terminal.observeStructuralIncomplete();
          }
        } catch {
          // Unknown unlabelled frames are extension/heartbeat territory.
        }
        continue;
      }
      try {
...
export const streamAnthropic: StreamFunction<"anthropic-messages", AnthropicOptions> = (
  ...
  void (async () => {
    ...
    try {
      ...
      let isOAuth: boolean;
      let getEndpointAuthority: () => AnthropicEndpointAuthoritySnapshot;
